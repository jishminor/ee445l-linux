...
static int16_t _width = ST7735_TFTWIDTH;  // this could probably be a constant,
                                          // except it is used in Adafruit_GFX
                                          // and depends on image rotation
...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w,
  ...
  int16_t originalWidth = w;  // save this value; even if not all columns fit on
                              // the screen, the image is still this width in
                              // ROM
  int i = w * (h - 1);
